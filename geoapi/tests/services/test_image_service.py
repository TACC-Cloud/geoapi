...
def test_get_exif_location(image_file_fixture):
    ...
    assert coordinates == GeoLocation(
        longitude=-80.78037499999999, latitude=32.61850555555556
    )
...
def test_process_image(image_file_fixture):
    ...
    assert imdata.coordinates == GeoLocation(
        longitude=-80.78037499999999, latitude=32.61850555555556
    )
...
def test_process_image_location_missing(image_file_no_location_fixture):
    ...
    imdata = ImageService.processImage(
        image_file_no_location_fixture, exif_geolocation=False
    )
