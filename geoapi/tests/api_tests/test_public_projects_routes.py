def test_get_project_using_single_uuid_unauthorized_guest(
    ...
    resp = test_client.get(
        "/public-projects/", query_string="uuid={}".format(projects_fixture.uuid)
    )
...
def test_get_public_project_using_single_uuid(test_client, public_projects_fixture):
    resp = test_client.get(
        "/public-projects/", query_string="uuid={}".format(public_projects_fixture.uuid)
    )
...
def test_put_project_not_found(test_client, projects_fixture):
    ...
    resp = test_client.put("/public-projects/{}/".format(projects_fixture), json=data)
...
def test_post_feature_not_found(test_client, projects_fixture, image_file_fixture):
    resp = test_client.post(
        "/public-projects/1/features/files/", data={"file": image_file_fixture}
    )
    assert resp.status_code == 404
