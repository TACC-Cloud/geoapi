...
def test_add_point_cloud_file(dbsession, projects_fixture, point_cloud_fixture,
    ...
    las_files = os.listdir(get_asset_dir(point_cloud.path, PointCloudService.ORIGINAL_FILES_DIR))
    ...
    assert len(os.listdir(get_asset_dir(point_cloud.project_id,
                                        point_cloud.feature.assets[0].path))) == 3  # index.html, pointclouds, libs
...
def test_delete_point_cloud_feature(celery_task_always_eager, dbsession, projects_fixture, point_cloud_fixture,
    ...
    point_cloud = dbsession.query(PointCloud).get(1)
    feature_asset_path = get_asset_dir(point_cloud.feature.assets[0].path)
    ...
    assert os.path.exists(get_asset_dir(point_cloud.path, PointCloudService.ORIGINAL_FILES_DIR))
