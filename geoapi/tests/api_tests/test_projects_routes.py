...
def test_get_projects_but_not_admin_or_creator(
    test_client, user2, projects_fixture2, projects_fixture
):
...
def test_get_projects_with_multiple(
    test_client, user1, projects_fixture2, projects_fixture
):
    resp = test_client.get("/projects/", headers={"X-Tapis-Token": user1.jwt})
    data = resp.json()
    assert resp.status_code == 200
...
def test_get_projects_using_uuids(
    test_client, projects_fixture, projects_fixture2, user1
):
    requested_uuids = [str(projects_fixture2.uuid), str(projects_fixture.uuid)]
    resp = test_client.get(
        "/projects/",
        params="uuid={}".format(",".join(requested_uuids)),
...
def test_get_projects_using_single_uuid(
    test_client, projects_fixture, projects_fixture2, user1
):
    resp = test_client.get(
        "/projects/",
        params="uuid={}".format(projects_fixture2.uuid),
        headers={"X-Tapis-Token": user1.jwt},
    )
    data = resp.json()
    assert resp.status_code == 200
...
def test_get_public_project_using_single_uuid(test_client, public_projects_fixture):
    resp = test_client.get(
        "/projects/", params="uuid={}".format(public_projects_fixture.uuid)
    )
    assert resp.status_code == 200


def test_get_project_using_single_uuid_unauthorized_guest(
...
def test_get_project_using_single_uuid_not_member_of_project(
    test_client, projects_fixture, user2
):
    resp = test_client.get(
        "/projects/",
        params="uuid={}".format(projects_fixture.uuid),
...
def test_project_data_single(test_client, projects_fixture, user1):
    resp = test_client.get(
        f"/projects/{projects_fixture.id}/", headers={"X-Tapis-Token": user1.jwt}
    )
    data = resp.json()
    assert resp.status_code == 200
...
def test_project_data_allow_public_access(test_client, public_projects_fixture, user2):
    ...
    resp = test_client.get(
        f"/projects/{public_projects_fixture.id}/", headers={"X-Tapis-Token": user2.jwt}
    )
    assert resp.status_code == 200


def test_delete_empty_project(
    test_client, projects_fixture, remove_project_assets_mock, user1, db_session
):
...
def test_delete_unauthorized(test_client, projects_fixture, user2, db_session):
    resp = test_client.delete(
        f"/projects/{projects_fixture.id}/", headers={"X-Tapis-Token": user2.jwt}
    )
    assert resp.status_code == 403
...
def test_delete_project_not_admin_or_creator(
    test_client, projects_fixture2, projects_fixture, user2
):
    resp = test_client.delete(
        f"/projects/{projects_fixture2.id}/", headers={"X-Tapis-Token": user2.jwt}
    )
    assert resp.status_code == 403
...
def test_get_users(test_client, projects_fixture, user1):
    resp = test_client.get(
        f"/projects/{projects_fixture.id}/users/", headers={"X-Tapis-Token": user1.jwt}
    )
    assert resp.status_code == 200
    assert resp.json() == [{"id": user1.id, "username": user1.username}]
...
def test_delete_user(test_client, projects_fixture2, user1, user2):
    resp = test_client.delete(
        f"/projects/{projects_fixture2.id}/users/{user2.username}/",
        headers={"X-Tapis-Token": user1.jwt},
    )
    assert resp.status_code == 204
...
def test_delete_user_unauthorized(test_client, projects_fixture, user2):
    resp = test_client.delete(
        f"/projects/{projects_fixture.id}/users/test1/",
        headers={"X-Tapis-Token": user2.jwt},
    )
    assert resp.status_code == 403
...
def test_get_point_clouds_listing(
    test_client, projects_fixture, point_cloud_fixture, user1
):
    resp = test_client.get(
        f"/projects/{projects_fixture.id}/point-cloud/",
        headers={"X-Tapis-Token": user1.jwt},
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_get_point_cloud(test_client, projects_fixture, point_cloud_fixture, user1):
    resp = test_client.get(
        f"/projects/{projects_fixture.id}/point-cloud/1/",
        headers={"X-Tapis-Token": user1.jwt},
    )
    assert resp.status_code == 200
...
def test_get_project_features_single_feature_public_access(
    test_client, public_projects_fixture, feature_fixture
):
    resp = test_client.get("/projects/{}/features/".format(public_projects_fixture.id))
    data = resp.json()
    assert resp.status_code == 200
...
def test_get_project_features_filter_with_assettype(
    test_client, projects_fixture, feature_fixture, image_feature_fixture, user1
):
    resp = test_client.get(
        f"/projects/{projects_fixture.id}/features/",
...
def test_get_project_features_filter_with_bounding_box(
    test_client, projects_fixture, feature_fixture, image_feature_fixture, db_session
):
    ...
    resp = test_client.get(
        f"/projects/{projects_fixture.id}/features/",
        params="bbox={}".format(",".join(map(str, bbox))),
...
def test_get_project_features_filter_with_date_range(
    test_client: "TestClient[Litestar]", projects_fixture, feature_fixture, user1
):
...
def test_update_project(test_client, projects_fixture, user1, db_session):
...
def test_create_project_watch_content_already_exists(
    ...
    resp = test_client.post(
        "/projects/", json=data, headers={"X-Tapis-Token": user1.jwt}
    )

    assert resp.status_code == 409
    assert (
        "Conflict, a project watching files for this storage system/path already exists"
...
def test_create_project_with_watch_content_watch_users(
    ...
    assert resp.status_code == 201
    resp_json = resp.json()
    assert resp_json["deletable"] is True
    assert resp_json["name"] == "Project name"
