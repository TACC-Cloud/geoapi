...
@pytest.fixture
def notifications(userdata, db_session):
...
@pytest.fixture
def progress_notifications(userdata, db_session):
    u1 = db_session.query(User).filter(User.username == "test1").first()
    u2 = db_session.query(User).filter(User.username == "test2").first()
    error = NotificationsService.createProgress(
        db_session, u1, "error", "test error", test_uuid1
    )
    success = NotificationsService.createProgress(
        db_session, u2, "success", "test success", test_uuid2
    )
    yield [error, success]
...
def test_filter_notifications(test_client, notifications, db_session):
    u1 = db_session.get(User, 1)
...
def test_filter_notifications_positive(test_client, notifications, db_session):
    u1 = db_session.get(User, 1)
...
def test_get_progress_notifications(test_client, progress_notifications, db_session):
    u1 = db_session.get(User, 1)
...
def test_delete_done_progress_notifications(
    test_client, progress_notifications, db_session
):
    u1 = db_session.get(User, 1)
...
def test_get_progress_notification(test_client, progress_notifications, db_session):
    u1 = db_session.get(User, 1)
...
def test_delete_progress_notification(test_client, progress_notifications, db_session):
    u1 = db_session.get(User, 1)
