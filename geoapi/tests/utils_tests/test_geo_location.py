...
def test_no_metadata(requests_mock, user1):
    ...
    assert get_geolocation_from_file_metadata(db_session, user1, system_id=SYSTEM, path=PATH) is None
...
def test_metadata_with_geolocation(requests_mock, user1, tapis_metadata_with_geolocation):
    ...
    assert (get_geolocation_from_file_metadata(db_session, user1, system_id=SYSTEM, path=PATH) ==
