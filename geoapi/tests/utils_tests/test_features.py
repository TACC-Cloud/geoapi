...
def test_is_supported_for_automatic_scraping_without_metadata():
    ...
    assert not features_util.is_supported_for_automatic_scraping_without_metadata(
        "foo.txt"
    )
    assert not features_util.is_supported_for_automatic_scraping_without_metadata(
        "foo.gif"
    ...
    assert features_util.is_supported_for_automatic_scraping_without_metadata("foo.JPG")
    assert features_util.is_supported_for_automatic_scraping_without_metadata(
        "foo.jpeg"
    )
    assert features_util.is_supported_for_automatic_scraping_without_metadata(
        "foo.JPEG"
...
def test_is_supported_file_type_in_rapp_folder_and_needs_metadata():
    # not supported type
    assert not features_util.is_supported_file_type_in_rapp_folder_and_needs_metadata(
        "/RApp/foo"
    )
    assert not features_util.is_supported_file_type_in_rapp_folder_and_needs_metadata(
        "/RApp/foo.txt"
    ...
    assert features_util.is_supported_file_type_in_rapp_folder_and_needs_metadata(
        "/RApp/foo.jpg"
    )
    assert features_util.is_supported_file_type_in_rapp_folder_and_needs_metadata(
        "/RApp/foo.JPG"
