...
def upgrade():
    ...
    op.add_column('tile_servers', sa.Column('kind', sa.String(), nullable=True, comment='Source kind for what the data is (e.g. cog)'))
    op.add_column('tile_servers', sa.Column('internal', sa.Boolean(), server_default='false', nullable=False, comment="True when served internally by our stack (e.g., TiTiler). When true, 'uuid' must be non-null. External layers keep this false."))
    op.add_column('tile_servers', sa.Column('uuid', sa.UUID(), nullable=True, comment="Identifier for internally managed assets (e.g., the generated COG file).  Required when 'internal' is true; null for external layers."))
    op.create_index(op.f('ix_tile_servers_uuid'), 'tile_servers', ['uuid'], unique=False)
