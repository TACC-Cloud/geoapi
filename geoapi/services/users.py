...
class UserService:
    ...
    @staticmethod
    def create(database_session, username: str, tenant: str, access_token: str = None) -> User:
        """
    ...
    def update_access_token(database_session, user: User, access_token: str) -> None:
        ...
        if not user.auth.access_token or jwt_utils.compare_token_expiry(access_token, user.auth.access_token):
            user.auth.access_token = access_token
            database_session.commit()

    @staticmethod
    def update_tokens(database_session,
                      user: User,
                      access_token: str, access_token_expires_at: int,
                      refresh_token: str, refresh_token_expires_at: int) -> None:
        """
    ...
    def refresh_access_token(database_session, user: User):
        ...
        if not user.has_unexpired_refresh_token():
            logger.error(f"Unable to refresh token for user:{user.username} tenant:{user.tenant_id}"
        ...
        try:
            ...
            with database_session.begin_nested():
                ...
                locked_auth = (database_session.query(Auth)
                               .filter(Auth.user_id == user.id)
                               .with_for_update()
                               .one())
                logger.info(f"Acquired auth for refreshing token for user:{user.username}"
                            f" tenant:{user.tenant_id}")
                ...
                if locked_auth.access_token_expires_at:
                    ...
                    if (locked_auth.access_token_expires_at - current_time) > buffer_time:
                        logger.info(f"No need to refresh token for user:{user.username}"
                ...
                if locked_auth.access_token_expires_at:
                    ...
                    access_token_expires_at = locked_auth.access_token_expires_at.replace(tzinfo=None)
                    buffer_time = timedelta(
                        seconds=jwt_utils.BUFFER_TIME_WHEN_CHECKING_IF_ACCESS_TOKEN_WAS_RECENTLY_REFRESHED)
                ...
                response = requests.put(f"{tapis_server}/v3/tokens",
                                        json=body)
                ...
                if response.status_code == 200 or response.status_code == 201:
                    ...
                    locked_auth.access_token_expires_at = data['access_token']['expires_at']
                    locked_auth.refresh_token = data['refresh_token']['refresh_token']
                    locked_auth.refresh_token_expires_at = data['refresh_token']['expires_at']
                    database_session.commit()
                    logger.info(f"Finished refreshing token for user:{user.username}"
                                f" tenant:{user.tenant_id}")
                else:
                    logger.error(
            ...
            logger.exception(f"Transaction error during token refresh for user:{user.username}: {str(ire)}")
            raise RefreshTokenError from ire
        except Exception as e:
            database_session.rollback()
            logger.exception(f"Error during token refresh for user:{user.username}: {str(e)}")
