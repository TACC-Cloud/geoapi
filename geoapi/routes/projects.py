...
feature_collection_model = api.model(
    "FeatureCollection",
    {
...
project_update_payload = api.model(
    "Project",
    {
        "name": fields.String(),
...
user_payload = api.clone("UserPayload", user, {"admin": fields.Boolean(default=False)})
...
tapis_file_upload_body = api.model(
    "TapisFileUpload", {"system_id": fields.String(), "path": fields.String()}
)

tapis_file = api.model(
    "TapisFile",
    {"system": fields.String(required=True), "path": fields.String(required=True)},
)

tapis_save_file = api.model(
    "TapisSaveFile",
    {
...
tapis_files_import = api.model(
    "TapisFileImport", {"files": fields.List(fields.Nested(tapis_file), required=True)}
)
...
overlay_parser.add_argument(
    "minLon", location=("form", "json"), type=float, required=True
)
overlay_parser.add_argument(
    "minLat", location=("form", "json"), type=float, required=True
)
overlay_parser.add_argument(
    "maxLon", location=("form", "json"), type=float, required=True
)
overlay_parser.add_argument(
    "maxLat", location=("form", "json"), type=float, required=True
)
...
class ProjectsListing(Resource):
    ...
    @api.doc(id="getProjects", description="Get a listing of projects", parser=parser)
    ...
    def get(self):
        ...
        if uuid_subset:
            ...
            subset = [
                check_access_and_get_project(
                    request.current_user, uuid=uuid, allow_public_use=True
                )
    ...
    @api.doc(id="createProject", description="Create a new project")
    @api.expect(project_payload)
    @api.marshal_with(project_response)
    @not_anonymous
    def post(self):
        u = request.current_user
        logger.info("Create project for user:{} : {}".format(u.username, api.payload))
...
class ProjectResource(Resource):
    ...
    @api.doc(id="getProjectById", description="Get the metadata about a project")
    @api.marshal_with(project_response)
    @project_permissions_allow_public
    ...
    @api.doc(id="updateProject", description="Update metadata about a project")
    @api.expect(project_update_payload)
    @api.marshal_with(project_response)
    @project_permissions
    def put(self, projectId: int):
        u = request.current_user
        logger.info("Update project:{} for user:{}".format(projectId, u.username))
    ...
    def post(self, projectId: int):
        ...
        logger.info(
            "Add user:{} to project:{} for user:{}".format(
                username, projectId, request.current_user.username
            )
        )
...
class ProjectUserResource(Resource):
    @api.doc(id="removeUser", description="Remove a user from a project")
    @project_admin_or_creator_permissions
    def delete(self, projectId: int, username: str):
        ...
        return ProjectsService.removeUserFromProject(db_session, projectId, username)
...
class ProjectFeaturesResource(Resource):
    ...
    def get(self, projectId: int):
        ...
        logger.info(
            f"Get features of project for user:{request.current_user.username} application:{application}"
    ...
    @api.doc(id="addGeoJSONFeature", description="Add a GeoJSON feature to a project")
...
class ProjectFeatureResource(Resource):
    @api.doc(id="getFeature", description="GET a feature of a project as GeoJSON")
    @api.marshal_with(api_feature)
    @project_permissions_allow_public
    def get(self, projectId: int, featureId: int):
        return FeaturesService.get(db_session, featureId)

    @api.doc(id="deleteFeature", description="GET a feature of a project as GeoJSON")
...
class ProjectFeaturesCollectionResource(Resource):
    ...
    def post(self, projectId: int, featureId: int):
        ...
        return FeaturesService.createFeatureAssetFromTapis(
            db_session, u, projectId, featureId, systemId, path
        )
...
class ProjectOverlaysResource(Resource):
    @api.doc(id="addOverlay", description="Add a new overlay to a project.")
...
class ProjectOverlaysImportResource(Resource):
    @api.doc(id="importOverlayFromTapis", description="Import an overlay from Tapis")
    ...
    def post(self, projectId: int):
        ...
        ov = FeaturesService.addOverlayFromTapis(
            db_session, u, projectId, systemId, path, bounds, label
        )
        return ov
...
class ProjectOverlayResource(Resource):
    ...
    @api.doc(id="removeOverlay", description="Remove an overlay from a project")
    @project_permissions
...
class ProjectStreetviewResource(Resource):
    @api.doc(
        id="addStreetviewSequenceToFeature",
    ...
    def post(self, projectId: int):
        ...
        return streetview.process_streetview_sequences(
            db_session, projectId, sequenceId, token
        )
...
class ProjectStreetviewFeatureResource(Resource):
    @api.doc(
        id="getStreetviewSequenceFromFeature",
...
class ProjectPointCloudsResource(Resource):
    ...
    @api.doc(id="addPointCloud", description="Add a point cloud to a project")
...
class ProjectPointCloudResource(Resource):
    ...
    @api.doc(id="getPointCloud", description="Get point cloud of a project")
    @api.marshal_with(point_cloud)
    @project_permissions_allow_public
    def get(self, projectId: int, pointCloudId: int):
        return PointCloudService.get(db_session, pointCloudId)

    @api.doc(id="updatePointCloud", description="Update point cloud")
    @api.marshal_with(point_cloud)
    @api.expect(point_cloud)
    @project_permissions
    @project_point_cloud_exists
    ...
    def put(self, projectId: int, pointCloudId: int):
        ...
        return PointCloudService.update(
            database_session=db_session, pointCloudId=pointCloudId, data=api.payload
        )
...
class ProjectPointCloudsFileImportResource(Resource):
    @api.doc(
        id="importPointCloudFileFromTapis",
...
class ProjectTasksResource(Resource):
    ...
    @api.doc(id="getTasks", description="Get a listing of all the tasks of a project")
...
class ProjectTileServersResource(Resource):
    @api.doc(id="addTileServer", description="Add a new tile server to a project.")
    ...
    @api.doc(id="updateTileServers", description="Update metadata about a tile servers")
    @api.marshal_with(tile_server, as_list=True)
    @project_permissions
    def put(self, projectId: int):
        u = request.current_user
        ...
        ts = FeaturesService.updateTileServers(
            database_session=db_session, dataList=api.payload
        )
        return ts
...
class ProjectTileServerResource(Resource):
    ...
    @api.doc(id="removeTileServer", description="Remove a tile server from a project")
    @project_permissions
    ...
    @api.doc(id="updateTileServer", description="Update metadata about a tile server")
    @api.marshal_with(tile_server)
    @project_permissions
    def put(self, projectId: int, tileServerId: int):
        ...
        return FeaturesService.updateTileServer(
            database_session=db_session, tileServerId=tileServerId, data=api.payload
        )
