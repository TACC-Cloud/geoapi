...
def on_project_creation(database_session, user: User, project: Project):
    ...
    try:
        ...
        logger.exception(
            f"Problem adding two default layers for user:{user.username}"
            f" project:{project.id} project_uuid:{project.uuid} "
        )
...
def on_project_deletion(database_session, user: User, project: Project):
    ...
    try:
        logger.debug(
            f"Removing .hazmapper file for user:{user.username}"
        ...
        logger.exception(
            f"Problem removing .hazmapper file for user:{user.username}"
            f"during deletion of project:{project.id} project_uuid:{project.uuid}"
            f"system_id:{project.system_id} file_path:{file_path}"
        )
    ...
    try:
        ...
        if project.system_id.startswith("project-"):
            ...
            update_designsafe_project_hazmapper_metadata(
                user, project, add_project=False
            )
    except Exception:
        logger.exception(
            f"Problem removing metadata for user:{user.username}"
            f" during deletion of project:{project.id} project_uuid:{project.uuid} "
        )
...
def update_designsafe_project_hazmapper_metadata(
    ...
    all_maps = []
    logger.debug(
        f"Current metadata for DesignSafe_project:{designsafe_uuid}: {current_metadata}"
    )
    if "hazmapperMaps" in current_metadata["baseProject"]["value"]:
        ...
        all_maps = [
            e
            for e in current_metadata["baseProject"]["value"]["hazmapperMaps"]
            if e["uuid"] != str(project.uuid)
        ]
    ...
    logger.debug(
        f"Updated metadata for DesignSafe_project:{designsafe_uuid}: {all_maps}"
    )
