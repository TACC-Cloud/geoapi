...
def publish(database_session, user: User, params: Dict, channels: "ChannelsPlugin"):
    ...
    if not streetview_service.token or not streetview_service.service_user:
        logger.exception(
            "Not authenticated to {} for user:{}".format(
                params["service"], user.username
            )
        )
    ...
    from_tapis_to_streetview.delay(
        user.id, streetview_service.id, system_id, path, organization_key
    )
...
def _from_tapis(
    database_session, user: User, task_uuid: UUID, systemId: str, path: str
):
    """
    ...
    for item in files_in_directory:
        ...
        if (
            ...
            not in features_util.IMAGE_FILE_EXTENSIONS
        ):
            continue
        try:
...
def _to_mapillary(
    ...
    try:
        ...
        MapillaryUtils.upload(
            database_session, user.id, task_uuid, service_user, organization_key
        )
    except Exception as e:
        msg = "Errors during mapillary upload task {} for user {}: Error: {}".format(
...
def check_existing_upload(
    session, user, streetview_service, task_uuid, system_id, path
):
    existing_progress = NotificationsService.getProgressUUID(session, task_uuid)
    existing_instance = StreetviewService.getInstanceFromSystemPath(
        session, streetview_service.id, system_id, path
    )
    # TODO: Handle existing progress
    if existing_progress or existing_instance:
        raise StreetviewExistsException(
            "Path {f} is already in progress or uploaded.".format(f=(system_id + path))
        )


# TODO: Ensure that just user works and not userid (previously took userid)
@app.task(queue="heavy")
def from_tapis_to_streetview(
    user_id: int,
    ...
    with create_task_session() as session:
        ...
        NotificationsService.createProgress(
            session,
            user,
            "created",
        ...
        streetview_instance = StreetviewService.createInstance(
            session, streetview_service.id, system_id, path
        )
        ...
        # TODO: Handle
            ...
            for streetview upload task: {} for user:{}. Error Message: {}".format(
                system_id, path, task_uuid, user.username, e
            )
            logger.exception(error_message)
            clean_session(
                session,
        ...
        if streetview_service.service == "mapillary":
            ...
            try:
                ...
                error_message = (
                    "Error during finalization of mapillary upload for streetview task:{} "
        ...
        NotificationsService.updateProgress(
            session, task_uuid, "success", "Finished Upload"
        )
...
def process_streetview_sequences(
    ...
    logger.info(
        "Starting streetview sequence processing task for sequence (#{}).".format(
            sequenceId
        )
    )

    convert_streetview_sequence_to_feature.apply_async(
        args=[projectId, sequenceId, token], task_id=celery_task_id
    )
...
class StreetviewSequenceProcessingTask(celery.Task):
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        logger.info(
            "Task ({}, streetview sequence {}) failed: {}".format(task_id, args, exc)
        )
        with create_task_session() as session:
...
def convert_streetview_sequence_to_feature(self, projectId, sequenceId, token):
    ...
    with create_task_session() as session:
        ...
        api_call_headers = {"Authorization": "OAuth " + token}

        sequence_response = requests.get(
            f"{mapillary_api_url}/image_ids?sequence_id={sequence_id}",
        ...
        if len(jsonResp) != 0:
            ...
            image_response = requests.get(
                f"{mapillary_api_url}/{image_id}?fields=thumb_1024_url",
                headers=api_call_headers,
            )
        ...
        for img in jsonResp:
            image_response = requests.get(
                f"{mapillary_api_url}/{img['id']}?fields=computed_geometry",
                headers=api_call_headers,
            )
            image_coordinates = (
                json.loads(image_response.content)
        ...
        logger.info(
            "Finished streetview sequence processing task for sequence (#{}).".format(
                sequenceId
            )
        )
