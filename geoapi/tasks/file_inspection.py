...
def inspect_files(
    ...
    """Expand ``path`` (file or directory) and classify each file on the worker.
    ...
    list of one.
    """
    from geoapi.services.file_inspection import classify, list_tapis_files

    with create_task_session() as session:
        user = session.get(User, user_id)
        client = TapisUtils(session, user)
        return [
            classify(client, system_id, p).model_dump()
