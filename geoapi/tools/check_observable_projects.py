...
with create_task_session() as session:
    ...
    logger.info(f"Checking observable_projects ({len(observable_projects)}) and projects ({len(projects)})")

    for obs in observable_projects:
        try:
            if not obs.project.watch_users or obs.watch_content != obs.project.watch_content:
                logger.info(f"Issue with observable project (id:{obs.id} project_id:{obs.project_id}):\n"
            ...
            logger.exception(f"Something happened when checking observable project"
                             f" (id:{obs.id} project_id:{obs.project_id})")

    logger.warn("Done checking.  Note some discrepancies could occur (i.e. projects are deleted but then deprecated"
